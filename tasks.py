...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_DIGITAL_TWIN_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_digital_twin.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_digital_twin.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_digital_twin.nautobot_ver,
        "PYTHON_VER": context.nautobot_digital_twin.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_digital_twin.project_name}",
        f'--project-directory "{context.nautobot_digital_twin.compose_dir}"',
    ]

    for compose_file in context.nautobot_digital_twin.compose_files:
        compose_file_path = os.path.join(context.nautobot_digital_twin.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_digital_twin.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_digital_twin.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_digital_twin.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_digital_twin.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_digital_twin.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_digital_twin.python_ver
    dockerfile_path = os.path.join(context.nautobot_digital_twin.compose_dir, "Dockerfile")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_digital_twin"
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_digital_twin"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_digital_twin") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_digital_twin.migrations"
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_digital_twin/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_digital_twin.local):
